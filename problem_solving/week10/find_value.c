...
void remove_length_Characters(char* sentence, int n) {
    int length = strlen(sentence);

    memmove(sentence + (n), sentence + (n)+2, length - (n)-1);
    sentence[length - 2] = '\0';
}

/* sentence에서 substring의 위치를 찾고, source의 맨앞부터 substring직전의 위치까지 destination에 복사하는 함수 */
...
char get_last_Character(char* sentence, int n) {
    int i = 0;

    while (sentence[i] != '\0') { // \0위치를 찾음
        i++;
    }
...
int main() {
    ...
    extract_quoted_Sentences(format_Sentence);
    extract_quoted_Sentences(value_Sentence);

    printf("형식문장 : %s\n", &format_Sentence);
    printf("  값문장 : %s\n\n", &value_Sentence);

    if (sentence_Test(value_Sentence) > 0) {
        printf("\n수정된 형식문장 : %s\n", &format_Sentence);
        printf("         값문장 : %s\n\n", &value_Sentence);
    }
    ...
    check_Count(format_Sentence, &int_Num, &string_Num, &char_Num, format_Sequence);
    int index = int_Num + string_Num + char_Num;
    ...
    for (int i = 0; i < string_Num; i++) { // 문자열에 해당하는 메모리 해제
        free(s[i]);
    }
